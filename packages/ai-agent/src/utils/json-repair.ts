...
let _repairMetrics: RepairMetrics = {
 ...
 */
export function getRepairMetrics(): RepairMetrics {
...
export function repairAndParseJSON(text: string): RepairResult {
  ...
  const fixers = [
    { name: "Remove trailing comma", fn: removeTrailingCommas, metric: null as keyof RepairMetrics | null },
    { name: "Close unterminated strings", fn: closeUnterminatedStrings, metric: "truncationRepairs" as keyof RepairMetrics },
    { name: "Balance brackets", fn: balanceBrackets, metric: "bracketBalances" as keyof RepairMetrics },
    { name: "Fix truncated arrays", fn: fixTruncatedArrays, metric: "truncationRepairs" as keyof RepairMetrics },
    { name: "Remove control characters", fn: removeControlCharacters, metric: null as keyof RepairMetrics | null },
  ];
 ...
 */
function fixTruncatedArrays(text: string): string {
 ...
 */
function aggressiveRepair(text: string): string | null {
 ...
 */
function normalizeEnumValues(data: unknown, repairs: string[]): unknown {
  if (data === null || data === undefined) return data;
  if (typeof data !== "object") return data;
  if (Array.isArray(data)) return data.map(item => normalizeEnumValues(item, repairs));
  ...
  for (const [key, value] of Object.entries(obj)) {
    if (key === "integrations" && typeof value === "object" && value !== null) {
      ...
      const normalizedIntegrations: Record<string, unknown> = {};

      for (const [intKey, intValue] of Object.entries(integrations)) {
        const validValues = VALID_ENUMS[intKey as keyof typeof VALID_ENUMS];
        if (validValues) {
      ...
      result[key] = value.map((route: unknown) => normalizeEnumValues(route, repairs));
    } else if (key === "components" && Array.isArray(value)) {
      // Normalize component objects
      result[key] = value.map((comp: unknown) => normalizeEnumValues(comp, repairs));
 ...
 */
function normalizeToEnum(value: string, validValues: string[]): string | null {
